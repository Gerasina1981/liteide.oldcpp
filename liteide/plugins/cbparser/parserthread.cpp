 ...
 *
 * $Revision: 5404 $
 * $Id: parserthread.cpp 5404 2009-01-16 09:38:59Z mortenmacfly $
 * $HeadURL: http://svn.berlios.de/svnroot/repos/codeblocks/trunk/src/plugins/codecompletion/parser/parserthread.cpp $
 */
/*
#include <sdk.h>
...
static void Delete(LoaderBase * loader)
{
...
namespace ParserConsts
{
	...
	const wxString kw_interface(_T("interface"));
...
void ParserThread::SkipFunctionBlock()
{
    // need to force the tokenizer _not_ skip anything
    // or else default values for template params would cause us to miss everything (because of the '=' symbol)
    bool oldState = m_Tokenizer.IsSkippingUnwantedTokens();
    m_Tokenizer.SetSkipUnwantedTokens(true);//false);
    ...
    // (since we 've read the {)
    unsigned int level = m_Tokenizer.GetNestingLevel() - 1;
	unsigned int index = m_Tokenizer.GetTokenIndex();
    while (1)
    {
        if(TestDestroy())
            return;
	...
	unsigned int end_index = m_Tokenizer.GetTokenIndex();
	wxString str = m_Tokenizer.GetBuffer().Mid(index,end_index-index-1);
		...
		if (token == _T("template"))
		{
			...
			if (token==ParserConsts::kw_class)
			{
				...
				if (m_Options.handleClasses)
					HandleClass(true,args);
				else
					SkipToOneOfChars(ParserConsts::semicolonclbrace, true);
				continue;
			}
			else if (token==ParserConsts::kw_struct || token == ParserConsts::kw_interface)
			{
				m_Str.Clear();
				if (m_Options.handleClasses)
					HandleClass(false,args);
				else
					SkipToOneOfChars(ParserConsts::semicolonclbrace, true);
				continue;
			}
            ...
            if (!peek.IsEmpty())
            {
               if (peek.GetChar(0) == '(' &&
                ...
                else if (peek==ParserConsts::lt)
                {
                    ...
                    {
//                        Manager::Get()->GetLogManager()->DebugLog(F(_T("peek='::', token='") + token + _T("', m_LastToken='") + m_LastToken + _T("', m_Str='") + m_Str + _T("'")));
                        if (m_Str.IsEmpty())
...
wxString ParserThread::GetActualTokenType()
{
    ...
    if (pos >= 0)
    {
        ...
        //                               const wxString&
        // in this example, we would stop here ^
        while (pos >= 0 && (wxIsalnum(m_Str.GetChar(pos)) || m_Str.GetChar(pos) == '_' || m_Str.GetChar(pos) == ':'))
            --pos;
...
//        Manager::Get()->GetLogManager()->DebugLog(F(_T("Created unknown class/namespace %s (%d) under %s (%d)"), ns.c_str(), newidx, parent ? parent->m_Name.c_str() : _T("<globals>"), parent ? parent->GetSelf() : -1));
    }
...
//    asm("int $3;");
    ...
    if (!newToken && !m_Options.isTemp)
        newToken = TokenExists(name+realargs, m_pLastParent, kind);

    if (newToken && newToken->m_TokenKind == kind && 
		(newToken->m_Args == args || (kind & tkAnyFunction && newToken->m_RealArgs == realargs) ))
    {
...
//                    Manager::Get()->GetLogManager()->DebugLog(F(_T("Special case \"#if 0\" not skipped.")));
        }
        m_Tokenizer.SkipToEOL();
    }
    else if (preproc==ParserConsts::kw_else || preproc==ParserConsts::kw_elif) // #else, #elif
    {
//        Manager::Get()->GetLogManager()->DebugLog(F(_T("Saving nesting level: %d"), m_Tokenizer.GetNestingLevel()));
...
//        Manager::Get()->GetLogManager()->DebugLog(F(_T("Restoring nesting level: %d (was %d)"), m_Tokenizer.GetNestingLevel(), l));
    }
...
void ParserThread::HandleNamespace()
{
    ...
    else
    {
        ...
        else if (next==ParserConsts::equals)
        {
            ...
            aliasToken->m_Aliases.push_back(ns);
        }
        else
        {
            m_Tokenizer.SetSkipUnwantedTokens(true);
...
void ParserThread::HandleClass(bool isClass, wxString template_args)
{
    // need to force the tokenizer _not_ skip anything
    ...
    while (1)
    {
		...
		{
			 m_Tokenizer.UngetToken();
			current = m_Tokenizer.GetToken(false,true);
			wxString args1 = m_Tokenizer.GetToken(false,true);
			if (template_args == _T("<>"))
			{
				template_args = _T("");
				current += next;
...
//        Manager::Get()->GetLogManager()->DebugLog(F(_T("Found class '%s'"), current.c_str()));
        ...
        if (!current.IsEmpty() && !next.IsEmpty())
        {
            ...
            {
//                Manager::Get()->GetLogManager()->DebugLog(F(_T("Class '%s' has ancestors"), current.c_str()));
                m_Tokenizer.GetToken(); // eat ":"
                while (1)
                {
                    wxString tmp = m_Tokenizer.GetToken();
                    ...
                    if (!(tmp==ParserConsts::comma || tmp==ParserConsts::gt))
                    {
                        ...
                        if (tmp==ParserConsts::dcolon || next==ParserConsts::dcolon)
                            ...
                            ancestors << tmp << _T(',');
//                        Manager::Get()->GetLogManager()->DebugLog(F(_T("Adding ancestor ") + tmp));
                    }
                    if (next.IsEmpty() ||
...
//                Manager::Get()->GetLogManager()->DebugLog(F(_T("Ancestors: ") + ancestors));
            ...
            if (current==ParserConsts::opbrace) // unnamed class/struct
            {
                ...
                Token* newToken = DoAddToken(tkClass, unnamedTmp, lineNr,0,0,template_args);
            ...
            {
                Token* newToken = DoAddToken(tkClass, current, lineNr,0,0,template_args);
                if (!newToken)
                {
...
//            Manager::Get()->GetLogManager()->DebugLog(F(_T("Ctor? '%s', m_Str='%s', localParent='%s'"), name.c_str(), m_Str.c_str(), localParent ? localParent->m_Name.c_str() : _T("<none>")));
        }
        else
        {
...
//        Log("Adding function '"+name+"': m_Str='"+m_Str+"'"+", enc_ns="+(m_EncounteredNamespaces.GetCount()?m_EncounteredNamespaces[0]:"nil"));
        ...
        while (!peek.IsEmpty()) // !eof
        {
            ...
            else if (peek == ParserConsts::opbrace)// function implementation
            {
                ...
                SkipFunctionBlock(); // skip  to matching }
                lineEnd = m_Tokenizer.GetLineNumber();
//                Manager::Get()->GetLogManager()->DebugLog(F(_T("Skipped function %s impl. from %d to %d"), name.c_str(), lineStart, lineEnd));
                break;
            }
    ...
    while (1)
    {
        ...
        if (peek==ParserConsts::comma || peek==ParserConsts::clbrace || peek==ParserConsts::colon)
        {
            ...
            if (wxIsalpha(token.GetChar(0)) || token.GetChar(0) == '_')
            {
				...
				//vfc wxString value = m_Tokens.GetEnumToken();
				wxString args = m_Tokenizer.GetToken(true);

                //DoAddToken(tkEnumerator, token, m_Tokenizer.GetLineNumber());
				DoAddToken(tkEnumerator, token, m_Tokenizer.GetLineNumber(),0,0,args);
                m_pLastParent = lastParent;
            }
...
//        Manager::Get()->GetLogManager()->DebugLog(F(_T("tdef: token=%s, peek=%s"), token.c_str(), peek.c_str()));
        ...
        if (token == ParserConsts::kw_union)
        {
            ...
            token == ParserConsts::kw_struct || token == ParserConsts::kw_interface)
        {
...
void ParserThread::HandleMapping(const wxString &token, const wxString &peek)
{
     DoAddToken(tkMapping, token, m_Tokenizer.GetLineNumber(),0,0,peek);
}
