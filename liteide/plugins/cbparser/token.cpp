 ...
 * $HeadURL: http://svn.berlios.de/svnroot/repos/codeblocks/trunk/src/plugins/codecompletion/parser/token.cpp $
 */
/*
#include <sdk.h>
#include <globals.h>
...
inline void SaveTokenIdxSetToFile(wxOutputStream* f,const TokenIdxSet& data)
{
    ...
    for (TokenIdxSet::const_iterator it = data.begin(); it != data.end(); it++)
    {
        int num = *it;
        SaveIntToFile(f, num);
    }
...
wxString Token::GetTokenKindString() const
{
    switch (m_TokenKind)
    {
		...
		case tkMapping: return _T("mapping");
